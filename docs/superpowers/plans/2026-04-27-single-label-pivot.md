...
> **2026-05-04 update:** Plan extended via the consolidated plan at `~/.claude/plans/ultraplan-session-creation-failed-happy-frost.md`. Branch is now `single-label-toggle` off `light-mode` (no worktree, no separate DB). Tasks 0a/0b (mode column + runtime toggle) precede Task 1. Tasks 11–13 (label queue, assignments, post-handoff summary) follow Task 10. The task bodies below remain authoritative for Tasks 1–10; ignore the original Task 0 (worktree setup) — replaced by mode-column + branch.
>
...
**Goal:** Pivot Chatsight from multi-label per-message labeling to single-label binary (yes/no/skip) classification with conversation-level cycling and per-label Gemini handoff. Both flows coexist via a runtime mode toggle.

**Architecture:** Branch `single-label-toggle` off `light-mode`. Same `chatsight.db`. `LabelDefinition` gains `mode`, `phase`, `is_active`, `queue_position`, `summary_json`. `LabelApplication` gains `value`. New `AssignmentMapping` table. `MessageCache` gains `notebook` + `assignment_id`. New `ConversationCursor`. Queue ordering walks one conversation start-to-end before jumping to the next. Single label "active" at a time within `mode="single"`. Gemini handoff fills remaining unlabeled messages for the active label and produces a summary; instructor reviews low-confidence predictions or refines and re-runs.
