...
- Optional: `CHATSIGHT_STUDY_LOCK=1` — opt-in study week-lock (`study_scope.py`): restricts `/queue` to Week 3 (Lab 1 + HW 1) and `/run` to Week 8 (Lab 5 + HW 5) of DSC 10 wi26. Off by default; main serves the full quarter.
- Optional: `CHATSIGHT_HYBRID_EXPLORE_FRACTION` — default explore-pick ratio for the single-label hybrid queue (default 0.75).
...
- Service modules (called from `main.py`):
  ...
  - `autolabel_service.py` — multi-label batch classification via Gemini function-calling (`classify_messages` tool, `mode=ANY`). The general auto-label pass is **multi-select** (a message can get several labels, gated by `CHATSIGHT_MULTILABEL_THRESHOLD`); the split-autolabel flow stays single-select. Candidates come from local `MessageCache` (archived labels and out-of-scope messages excluded), not the external DB.
  ...
  - `explore_service.py` — hybrid-queue "explore" sampling: embeds *student* messages only (tutor turns excluded) to surface rare/specific help requests over generic "help"/assignment-prompt spam. Backs `hybrid_explore_fraction` (env default `CHATSIGHT_HYBRID_EXPLORE_FRACTION`, 0.75).
  ...
  - `name_suggestion_service.py` — label-name autocomplete: filters concept-derived candidate names by cosine similarity against existing label names (backs `GET /api/concepts/name-suggestions` and the `LabelNameInput` dropdown).
  - `onboarding_service.py` — picks a high-quality starter conversation + suggested label names for first-run onboarding; also provides the seed that lets `queue_service` skip a full `MessageCache` scan for brand-new labels.
  - `study_scope.py` — opt-in study week-lock (see `CHATSIGHT_STUDY_LOCK` above); inert by default. `in_scope_keys` results are memoized per (lock, scope, row-count).
...
- `KeybindProvider`/`useKeybinds` (`src/hooks/useKeybinds.tsx`) — single-label keyboard shortcuts (yes/no/skip/undo), defaults yes=`a`, no=`d`, skip=`ArrowRight`, undo=`z`, persisted to `localStorage` and editable via `KeybindSettingsModal`.
...
- `/summaries` and `/analysis` are **mode-aware**: each page picks a multi- or single-label variant at runtime (`SummariesPageMulti`/`SummariesPageSingle`, `MultiLabelAnalysis`/`SingleLabelAnalysis`). Single-label summaries also show a human-vs-AI provenance glyph (`AppliedByGlyph`), poll live progress while a handoff is classifying, and surface `skip` as a distinct verdict alongside yes/no/review.
...
- **Hybrid explore sampling**: the single-label queue mixes ordinary picks with `explore_service` "explore" picks (student-message novelty) at ratio `hybrid_explore_fraction` (per-label override, else env `CHATSIGHT_HYBRID_EXPLORE_FRACTION`, default 0.75) to avoid drowning in generic "help" spam.
