...
// ── Label Messages Modal ─────────────────────────────────────────────────────

function LabelMessagesModal({ label, onClose }: { label: LabelDefinition; onClose: () => void }) {
  const [examples, setExamples] = useState<LabelExample[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    api.getLabelExamples(label.id, 200)
      .then(setExamples)
      .catch(() => {})
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay backdrop-blur-sm p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-edge-subtle shrink-0">
          <div>
            <h2 className="text-sm font-bold text-on-canvas">{label.name}</h2>
            <p className="text-[10px] text-faint mt-0.5">{label.count} labeled message{label.count !== 1 ? 's' : ''}</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded hover:bg-elevated text-faint hover:text-on-surface transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
          {loading ? (
            <div className="space-y-2">
              {[1,2,3].map(i => <div key={i} className="h-14 bg-elevated rounded animate-pulse" />)}
            </div>
          ) : examples.length === 0 ? (
            <p className="text-sm text-disabled text-center py-8">No messages labeled yet.</p>
          ) : examples.map((ex, i) => (
            <div key={i} className="bg-message-student border border-message-border rounded px-3 py-2.5">
              <p className="text-[9px] text-accent-text uppercase tracking-wide mb-1">msg {ex.message_index} · chatlog {ex.chatlog_id}</p>
              <p className="text-xs text-on-surface leading-relaxed">{ex.message_text}</p>
            </div>
          ))}
        </div>
      </div>
...
export function LabelsPage() {
  ...
  const [viewingMessages, setViewingMessages] = useState<LabelDefinition | null>(null)
