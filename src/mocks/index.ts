...
export const mockApi = {
    ...
    labels: [
      {
        label_id: 1,
        label_name: 'Concept Question',
        description: 'Student asks about a course concept',
        human_count: 45,
        ai_count: 17,
        total_count: 52,
        high_conf_pct: 82,
        ...
        human_pct: 87,
        updated_at: '2026-05-08T14:30:00Z',
        weekly_sparkline: [20, 35, 42, 38, 55, 60, 48, 52],
      },
      {
        label_id: 2,
        label_name: 'Debug Help',
        description: 'Student needs help fixing code',
        human_count: 28,
        ai_count: 12,
        total_count: 32,
        high_conf_pct: 75,
        ...
        weekly_sparkline: [15, 22, 30, 28, 32, 25, 20, 18],
      },
    ],
  } satisfies import('../types').MultiLabelCohortResponse,
  ...
  multiLabelDetail: {
    ...
    confidence_histogram: {
      bins: [
        { lo: 0.0, hi: 0.1, count: 1 },
        { lo: 0.1, hi: 0.2, count: 0 },
        { lo: 0.2, hi: 0.3, count: 1 },
        { lo: 0.3, hi: 0.4, count: 2 },
        { lo: 0.4, hi: 0.5, count: 1 },
        { lo: 0.5, hi: 0.6, count: 2 },
        { lo: 0.6, hi: 0.7, count: 3 },
        { lo: 0.7, hi: 0.8, count: 4 },
        { lo: 0.8, hi: 0.9, count: 2 },
        { lo: 0.9, hi: 1.0, count: 1 },
      ],
      coverage: { with_confidence: 17, total_ai: 17 },
    },
    provenance: {
      human_applications: 45,
      ai_applications: 17,
      human_pct: 73,
    },
    position_distribution: [
      { bucket: 'early', count: 22, pct: 34 },
      { bucket: 'mid', count: 18, pct: 28 },
      { bucket: 'late', count: 25, pct: 38 },
    ],
    by_assignment: [
      { key: 'Lab 2', human: 18, ai: 6, total: 24, human_pct: 75 },
      { key: 'Lab 5', human: 12, ai: 5, total: 17, human_pct: 71 },
      { key: 'Unassigned', human: 15, ai: 6, total: 21, human_pct: 71 },
    ],
    co_occurring_labels: [
      { label_name: 'Debug Help', count: 14, pct: 27 },
      { label_name: 'Clarification', count: 9, pct: 17 },
    ],
    by_hour_of_day: Array.from({ length: 24 }, (_, hour) => ({
      hour,
      count: hour >= 10 && hour <= 22 ? 3 + (hour % 4) : 0,
    })),
    examples: {
      human: [
        {
          ...
          chatlog_id: 11,
          message_index: 2,
          text: 'What does groupby actually do here?',
          applied_by: 'human',
          confidence: null,
          assignment: 'Lab 2',
          position_bucket: 'early',
          co_labels: ['Clarification'],
      ...
      low_confidence: [
        {
          ...
          chatlog_id: 12,
          message_index: 4,
          text: 'Is this the right way to merge DataFrames?',
          applied_by: 'ai',
          confidence: 0.62,
          assignment: 'Lab 5',
          position_bucket: 'mid',
  ...
  } satisfies import('../types').MultiLabelDetail,
